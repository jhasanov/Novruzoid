...
abstract class Kernel extends QMatrix {
    ...
    static double k_function(svm_node[] x, svm_node[] y, int ylen,
                             svm_parameter param) {
        switch (param.kernel_type) {
            ...
            case svm_parameter.RBF: {
                double sum = 0;
                int xlen = x.length;
                //int ylen = y.length; Commented by Jamal (It will be passed as parameter)
                int i = 0;
                int j = 0;
                while (i < xlen && j < ylen) {
                    if (x[i].index == y[j].index) {
...
public class svm {
    ...
    public static double[] svm_predict_values(svm_model model, svm_node[] x, double[] dec_values) {
        ...
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
            ...
            double sum = 0;
            for (i = 0; i < model.l; i++)
                sum += sv_coef[i] * Kernel.k_function(x, model.SV[i], model.colIdx[i], model.param);
            sum -= model.rho[0];
            dec_values[0] = sum;

            if (model.param.svm_type == svm_parameter.ONE_CLASS) {
                double val = (sum > 0) ? 1 : -1;
                return new double[]{0.0, val};
            } else {
            ...
            for (i = 0; i < l; i++) {
                kvalue[i] = Kernel.k_function(x, model.SV[i], model.colIdx[i], model.param);
            }
